...
function Login() {
  ...
  return <main className="center-screen login"><Brand /><section><p className="eyebrow">ليلة الكامب</p><h1>ادخل على مهمتك</h1><p className="muted">الحساب بيحدد صلاحياتك تلقائيًا.</p><form onSubmit={submit} className="stack-form"><label>اسم المستخدم<input dir="ltr" type="text" autoCapitalize="none" autoComplete="username" spellCheck={false} required value={identity} onChange={(e) => setIdentity(e.target.value)} /></label><label>كلمة السر<input dir="ltr" type="password" autoComplete="current-password" required value={password} onChange={(e) => setPassword(e.target.value)} /></label>{mutation.error && <div className="inline-alert error" role="alert"><AlertTriangle size={19} />تعذر تسجيل الدخول. راجع البيانات وحاول تاني.</div>}<button className="primary-button" disabled={mutation.isPending}>{mutation.isPending ? <><LoaderCircle className="spin" />جاري الدخول</> : 'دخول'}</button></form></section></main>
}
